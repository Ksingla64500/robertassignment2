...
let time = 0; // Start from 0 for stopwatch and lap timer
let interval;
let isRunning = false;
let laps = [];
...
function formatTime(ms) {
    ...
    return date.toISOString().substr(11, 8); // Changed to display mm:ss
}
...
function startStop(timerId) {
    ...
    if (isRunning) {
        ...
        interval = setInterval(() => {
            time += 10; // Increment by 10ms for better accuracy
...
setInterval(() => {
    if (alarmTime && new Date().toTimeString().startsWith(alarmTime)) {
        document.getElementById('alarm-display').textContent = 'ALARM!';
        document.getElementById('alarm-display').classList.add('alarm-ringing');
        playAlarmSound();
    }
